...
const Logout = () => {
  ...
  return (
    <div className="mt-[120px] mb-[160px] w-[540px] mx-auto bg-mediumDarkContrast p-6 rounded-lg">
      <h1 className="pt-0.5 text-3xl mb-5">Log out of this session</h1>
      <div className="w-full h-[2px] bg-grey my-6" />
      <p className="text-grey40 mb-6 mt-4 flex items-center gap-2">
      ...
      <div className="flex gap-3">
        ...
        {loginForm._seedPhrase.length > 0 && (
          <div
            role="button"
            onClick={() => handleCopy(loginForm._seedPhrase, "seed")}
            className={`flex select-none items-center active:scale-[95%] gap-3 text-sm bg-darkContrast bg-opacity-30 rounded-lg w-fit px-6 transition-all ${copied ? "text-main" : "text-white"}`}
          >
            Copy
            <svg
              width="13"
              ...
              className={`${copied ? "fill-main" : "fill-white"} transition-all h-[20px]`}
            >
              <path
        ...
        <div className="flex gap-3">
          <span>
            ...
            <strong className="font-bold">
              {_keyUsages[_address] ? _keyUsages[_address] : 1}
            </strong>
          </span>
